...
function dist(a: PointerPos, b: PointerPos) {
  return Math.hypot(a.x - b.x, a.y - b.y);
}
function midpoint(a: PointerPos, b: PointerPos): PointerPos {
...
export function usePuzzleCanvas(args: UsePuzzleCanvasArgs) {
  ...
  // Resize observer: keep canvas buffer in sync with CSS size + DPR.
  useEffect(() => {
  ...
  // Re-fit whenever a new puzzle/engine instance is loaded.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      const canvas = canvasRef.current;
      const { engine, image, showGhost, highContrast, backgroundColor } = argsRef.current;
    ...
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (pinchRef.current && pointers.current.size >= 2) {
        ...
        const targetScale = clamp(
          pinch.startScale * (d / pinch.startDist),
          fitScaleRef.current * 0.4,
  ...
  const zoomBy = useCallback((factor: number) => {
    ...
    viewportRef.current = zoomViewportAt(viewportRef.current, cx, cy, target);
  }, []);
